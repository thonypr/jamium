public final class Responses {

    public static final String WELCOME = "Привет, X!\nЗдесь ты можешь проверить свои ответы на задания Jamium!\n" +
            "Если нужна помощь по заданию - просто отправь мне сообщение вида: /help и свой вопрос";
    public static final String TASK_1 = "Формат ответа: число\n⠋⠊⠗⠎⠞⠏⠗⠊⠍⠑⠁⠋⠞⠑⠗⠍⠊⠇⠇⠊⠕⠝";
    public static final String CONGRAT_1 = "Верно!\nШрифт Брайля просит найти первое простое число после 1000000,\n" +
            "а это именно 1000003";
    public static final String CLOSE_1 = "Почти! Ещё немного и будет верный ответ!";
    public static final String TASK_2 = "Формат ответа: число";
    public static final String CLOSE_2 = "На верном пути!";
    public static final String CONGRAT_2 = "Верно!\n https://www.youtube.com/watch?v=G2_Q9FoD-oQ";
    public static final String TASK_3_1 = "Формат ответа: число (13 цифр)";
    public static final String TASK_3_2 = "Формат ответа: строка";
    public static final String CONGRAT_3_1 = "Верно!\n Верно";
    public static final String CONGRAT_3_2 = "Верно!\nПотому что главное сокровище - это ты!";
    public static final String CLOSE_3_1 = "На верном пути!";
    public static final String CLOSE_3_2 = "На верном пути!";
    public static final String TASK_4_1 = "Формат ответа: слово (регистр не имеет значения, 10 букв)";
    public static final String TASK_4_2 = "Формат ответа: число";
    public static final String CONGRAT_4_1 = "Верно!\n Верно";
    public static final String CONGRAT_4_2 = "Верно!\nРечь шла о Маяковском и станции метро Маяковская в Санкт-Петербурге, " +
            "где высечены слова из его поэмы 'Хорошо!'\n" +
            "А дальше нужно было набрать слово 'хорошо' в русской раскладке на кнопочном телефоне!\n" +
            "Рассказать о впечатлениях от задания можно мне в личку, \nбуду очень благодарен за обратную связь: @aprimako";
    public static final String CLOSE_4_1 = "Да, речь идёт о питерских трамваях!\nНужно понять, что дальше делать с этим :)";
    public static final String CLOSE_4_2 = "Да, сейчас речь идёт о метрополитене!";
    public static final String TASK_5 = "Формат ответа: число";
    public static final String TASK_5_TEXT = "Один за всех - и все за одного!";
    public static final String TASK_5_1 = "Формат ответа: число";
    public static final String TASK_5_2 = "Формат ответа: число";
    public static final String TASK_5_3 = "Формат ответа: число";
    public static final String CONGRAT_5 = "Да, всё так! Задание решено и оно последнее!\nЕсли хочешь, можешь написать что-нибудь мне" +
            " в виде обратной связи - так задания будут становитья интереснее! \nА, и ещё:";
    public static final String CONGRAT_5_1 = "Да, речь шла о Футураме! И нужно было просто сказать, " +
            "в каком году начали в России отмечать Новый Год 1 января!\n" +
            "Самое время приступить к остальным частям задания!";
    public static final String TASK_5_1_TEXT = "Сидишь ты дома в кресле, пьёшь себе спокойно чай,\n" +
            "и вдруг - яркая вспышка!\n" +
            "Появляется портал, в котором стоят робот, парень и девушка.\n" +
            "У девушки почему-то один глаз...\n" +
            "Тебе кажется, что они тебе знакомы, но ты сомневаешься...\n" +
            "\n" +
            "Робот говорит что-то, бросает тебе записку и портал закрывается.\n" +
            "\n" +
            "Хорошо бы разобраться, чего они от тебя хотели?";
    public static final String TASK_5_2_TEXT = " : 10^7";
    public static final String CONGRAT_5_2 = "Да, в стереограмме 20! - ведь наступает 2020 год!" +
            " Ну и поделить нужно для красоты :)\n" +
            "Самое время приступить к остальным частям задания!";
    public static final String TASK_5_3_TEXT = "Как наверное было бы здорово всем людям понимать друг друга независимо от языка...";
    public static final String CONGRAT_5_3 = "Да, это язык Блисс, и указать нужно было количество часов в уходящем году! " +
            "Не теряй их зря!" +
            "\nСамое время приступить к остальным частям задания!";


    private Responses() { }
}